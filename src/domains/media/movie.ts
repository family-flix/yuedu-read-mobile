...
type TheTypesOfEvents = {
  [Events.ProfileLoaded]: {
    ...
    curSource: CurNovelChapter;
  };
  [Events.SourceFileChange]: NovelChapterSourceFile & { currentTime: number };
...
export class MovieMediaCore extends BaseDomain<TheTypesOfEvents> {
  ...
  curSource: CurNovelChapter | null = null;
  /** 当前影片播放进度 */
  currentTime = 0;
  /** 手动调整播放进度差值，用于校准字幕 */
  currentFixTime = 0;
  ...
  async fetchProfile(media_id: string) {
    ...
    const res = await fetch.run({ novel_id: media_id, type: MediaTypes.Movie });
    if (res.error) {
      const msg = this.tip({ text: ["获取电视剧详情失败", res.error.message] });
      return Result.Err(msg);
    }
    const { id, name, overview, sourceCount, coverPath: posterPath, curSource } = res.data;
    this.profile = {
      id,
      name,
      overview,
  ...
  updatePlayProgressForce(values: Partial<{ currentTime: number; duration: number }> = {}) {
    ...
    request.run({
      ...
      novel_chapter_id: this.curSource.id,
